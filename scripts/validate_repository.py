...
def validate_topics() -> None:
    ...
    for topic in topics:
        ...
        preview_files = [p for p in images.iterdir() if p.suffix.lower() in {".png", ".jpg", ".jpeg", ".svg"}]
        ...
        # These are study notes, not metadata stubs. Keep a meaningful floor and a readable learning structure.
        ...
        if "самопровер" not in note_text.casefold(): fail(f"{notes[0].relative_to(ROOT)}: missing self-check section")
