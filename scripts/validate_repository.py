...
def validate_topics() -> None:
    ...
    if not topics: fail("No topic directories found")
    for topic in topics:
        match = TOPIC_RE.match(topic.name); assert match is not None
        number = match.group(1); number_int = int(number)
        readme, notebooks, images = topic / "README.md", topic / "notebooks", topic / "images" / "previews"
        ...
        if not any(p.suffix.lower() in {".png", ".jpg", ".jpeg", ".svg"} for p in images.iterdir()): fail(f"{topic.name}: no preview images found")
        ...
        if len(note_text.strip()) < 5000: fail(f"{notes[0].relative_to(ROOT)}: study summary is too short (<5000 chars)")
        headings = re.findall(r"^##\s+", note_text, flags=re.MULTILINE)
        if len(headings) < 12: fail(f"{notes[0].relative_to(ROOT)}: expected at least 12 learning sections")
        if "главная идея" not in note_text.casefold(): fail(f"{notes[0].relative_to(ROOT)}: missing main-idea section")
        # Topic 01–02 predate the standardized self-check template; all later notes must contain it.
        if number_int >= 3 and "самопровер" not in note_text.casefold(): fail(f"{notes[0].relative_to(ROOT)}: missing self-check section")
...
def main() -> int:
    validate_topics(); validate_relative_markdown_links(); validate_obsidian(); print("Repository validation passed."); return 0
